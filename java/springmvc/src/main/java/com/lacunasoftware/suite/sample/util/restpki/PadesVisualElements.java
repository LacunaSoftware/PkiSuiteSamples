package com.lacunasoftware.suite.sample.util.restpki;
...
public class PadesVisualElements {
	...
	public static PadesVisualRepresentation getVisualRepresentation() throws IOException, RestException {
		...
		PadesVisualImage image = new PadesVisualImage(StorageMock.getPdfStampContent(), "image/png");
	...
	public static PdfMark getPdfMark(int sampleNumber) {
		...
		switch (sampleNumber) {
			case 1:
				...
				// We'll use the image at 'content/stamp.png'.
				try {
					elementImage.setImage(new PdfMarkImage(StorageMock.getPdfStampContent(), "image/png"));
				} catch (IOException e) {
			...
			case 2:
				...
				// We'll use the 'Content/stamp.png' as background.
				try {
					elementImage.setImage(new PdfMarkImage(StorageMock.getPdfStampContent(), "image/png"));
				} catch (IOException e) {
					throw new RuntimeException("Error trying to recovery the PDF stamp");
