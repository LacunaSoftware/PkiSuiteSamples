...
@Controller
public class PrinterFriendlyPadesRestController {
	...
	@RequestMapping(value = "/printer-friendly-pades-rest", method = {RequestMethod.GET})
	public void get(
		...
		InputStream fileStream = StorageMock.openRead(fileId);
		...
		byte[] pfvContent = generatePrinterFriendlyVersion(fileStream, verificationCode);
	...
	private byte[] generatePrinterFriendlyVersion(InputStream pdfStream, String verificationCode) throws IOException, RestException {
		...
		BlobReference blob = client.uploadFile(pdfStream);
			...
			manifestMark
					...
					.addElement(
							pdf.imageElement()
									...
									.withImage(StorageMock.getValidationResultIconContent(signer.getValidationResults().isValid()), "image/png"))
