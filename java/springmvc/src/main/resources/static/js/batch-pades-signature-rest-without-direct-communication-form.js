...
var batchSignatureRestForm = (function () {
	...
	function startSignature(step, done) {
		...
		$.ajax({
			...
			method: 'POST',
			contentType: 'application/json', // Ensure JSON payload
			data: JSON.stringify({
				signerCertificate: selectedCertContent,
				docId: step.docId
			}),
			dataType: 'json',
		}).then(function (signatureStartResult) {
			console.log('signatureStartResult: ', signatureStartResult);
			let result = JSON.parse(signatureStartResult);
			// Add the token to the document information (we'll need it in the second step).
			step.token = result.token;
			step.toSignHash = result.toSignHash;
			step.digestAlgorithmOid = result.digestAlgorithmOid;
	...
	function completeSignature(step, done) {
		...
		$.ajax({
			...
			data: JSON.stringify({
				token: step.token,
				signatureBase64: step.signatureBase64
			}),
			dataType: 'json',
