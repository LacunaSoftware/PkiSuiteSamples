// ------------------------------------------------------------------------------------------------
// This file contains logic for calling the Web PKI component to sign a batch of documents (Core scaffolding).
// It mirrors the Express sample flow and should be adapted as needed.
// ------------------------------------------------------------------------------------------------
var batchSignatureCoreForm = (function () {
		...
		for (var i = 0; i < formElements.documentsIds.length; i++) {
			...
			formElements.docList
				.append($('<li />')
					.append($('<a />')
						.text('Document ' + docId)
						.attr('href', '/download/doc?fileId=' + docId)));
		}
	...
	function startBatch() {
		...
		//
		//    1. Start the signature    : Call the action batch-pades-signature-core/start to start the
		...
		//                                the start action.
		//    3. Complete the signature : Call the action batch-pades-signature-core/complete to notify
	...
	// batch-pades-signature-core/start in order to start the signature and get the token associated
	...
	function startSignature(step, done) {
		...
		$.ajax({
			...
			success: function (response) {
				...
				// third steps).
				step.thumbprint = formElements.certificateSelect.val();
				step.state = response.state;
				step.hash = response.hash;
	...
	// to the action "complete" in order to complete the signature.
	//
	...
	function completeSignature(step, done) {
		...
		$.ajax({
			...
			success: function (response) {
				...
				step.filename = response.filename;	
				step.location = response.location;
				// Render success.
				renderSuccess(step);
				// Call the "done" callback signalling we're done with the document.
				done(step);
	...
	function renderSuccess(step) {
		...
		docLi
			...
			.append(document.createTextNode(' '))
			.append($('<a />')
				.text(step.filename.replace('_', '.'))
				.attr('href', step.location));
	}
