...
function generatePrinterFriendlyVersion(pdfPath, verificationCode) {
	...
	return new Promise((resolve, reject) => {
		...
		sigExplorer.open().then((signature) => {
			...
			const signerNames = Util.joinStringPt(signerNamesList);
			...
			pdfMarker.marks.push(
				pdf.mark()
					.onAllPages()
					.onContainer(pdf.container().width(1).anchorRight(1).height(1)
						.anchorBottom(1))
					.addElement(
						pdf.imageElement()
							.withOpacity(75)
			...
			pdfMarker.marks.push(
				pdf.mark()
					.onAllPages()
					.onContainer(pdf.container().height(2).anchorBottom().varWidth()
						.margins(1.5, 3.5))
			...
			pdfMarker.marks.push(
				pdf.mark()
					.onAllPages()
					.onContainer(pdf.container().width(2).anchorRight().varHeight()
						.margins(1.5, 3.5))
					.addElement(
						pdf.textElement()
