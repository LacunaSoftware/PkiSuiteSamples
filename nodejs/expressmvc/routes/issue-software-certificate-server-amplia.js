...
router.post('/', async (req, res, next) => {
	try {
		...
		// created order's id.
		let cert = await client.issueSoltwareCertificate(order.getId(), "1234", 2048);


		console.log("certificatePem",cert._certificatePem)

		console.log("privateKeyPkcs8Pem",cert._privateKeyPkcs8Pem)
		...
		// Store certificate.
		StorageMock.storeSync(cert._certificatePem, certId, '.pem');

		StorageMock.storeSync(cert._privateKeyPkcs8Pem, pKcs8Id, '.pem');
		...
		// // Store certificate.
		// StorageMock.storeSync(cert, certId, '.pfx');

		// Render complete page. We pass the certId responsible for locate the
		// key and certificate files.
		res.render('issue-software-certificate-server-amplia/complete', {
			certId: certId,
			pKcs8Id:pKcs8Id
		});
