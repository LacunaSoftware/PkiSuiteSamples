...
/**
 * This file initiates a CAdES signature using REST PKI and renders the signature page. The form is
 * posted to another file, cades-signature-action.php, which calls REST PKI again to complete the
 * signature.
 *
...
// Instantiate the CadesSignatureStarter class, responsible for receiving the signature elements and
...
if (!empty($cmsToCoSign)) {
    ...
    // CMS. We'll set the path to the CMS to be co-signed, which was previously saved in the
    // "app-data" folder by the file cades-signature/complete.php. Note two important things:
    //
    //      1. The CMS to be co-signed must be set using the method "setCmsToSign" or
    //         "setCmsFileToSign", not the method "setContentToSign" nor "setFileToSign".
    //
...
// parameter is omitted, the following rules apply:
...
// this value to call the signWithRestPki() method on the Web PKI component (see javascript below)
// and also to complete the signature after the form is submitted (see file
// cades-signature-action.php). This should not be mistaken with the API access token.
