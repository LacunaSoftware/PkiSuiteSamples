...
$verificationCode = StorageMock::getVerificationCode($fileId);
if (!isset($verificationCode)) {
    // If not, generate a code and register it
    $verificationCode = StorageMock::generateVerificationCode();
...
$pdfPath = StorageMock::APP_DATA_PATH . StorageMock::formatVerificationCode($verificationCode) . '.pdf';
StorageMock::createAppData();
$putContents = file_put_contents($pdfPath, $pfvContent);
if(!$putContents){
    throw new Exception($e);
}
$filename = StorageMock::formatVerificationCode($verificationCode) . '.pdf';
...
function generatePrinterFriendlyVersion($pdfPath, $verificationCode)
{
    ...
    // verification page).
    $formattedVerificationCode = StorageMock::formatVerificationCode($verificationCode);
    ...
    $signatureExplorer->securityContext = Util::getSecurityContextId();
    ...
    $signerNames = Util::joinStringsPt($certDisplayNames);
    $allPagesMessage = 'Este documento foi assinado digitalmente por ' . $signerNames
        . '.' . $breakline . 'Para verificar a validade das assinaturas acesse ' . $verificationSiteNameWithArticle
    ...
    $element->opacity = 75;
    $element->image = new PdfMarkImage(StorageMock::getIcpBrasilLogoContent(), 'image/png');
    ...
    foreach ($signature->signers as $signer) {
        ...
        $element->image = new PdfMarkImage(StorageMock::getValidationResultIcon($signer->validationResults->isValid()), 'image/png');
        array_push($manifestMark->elements, $element);
...
<div class="container content">
    ...
    <h2 class="ls-title">Generate a printer-friendly version from a PDF signature</h2>
    <h5 class="ls-subtitle">File generated successfully! <i class="fas fa-check-circle text-success"></i></h5>
